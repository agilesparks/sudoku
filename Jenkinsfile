    ...
    stages {
        ...
        stage('E2E Tests') {
             steps {
                ...
                sh 'docker run --rm -e "PORT=4001" -d --name e2ebackend backendforjenkins'
                sh 'docker run --rm -e "PORT=4000" -d --name e2efrontend -e "BACKEND_PORT=4000" frontendforjenkins'
                sh 'docker run  -e "CYPRESS_baseUrl=http://localhost:4000/" -v $PWD/frontend:/e2e -w /e2e cypress/included:3.2.0'
